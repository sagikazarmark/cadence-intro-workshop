...
start: docker-compose.override.yml ## Start services
	...
	mkdir -p var/docker/volumes/mysql
	docker-compose up -d
...
.PHONY: shell
shell: ## Start a shell with the Cadence CLI
	docker-compose exec -e CADENCE_CLI_ADDRESS=cadence:7933 -e CADENCE_CLI_DOMAIN=workshop cadence bash
